...
--
-- Model: a match is fully described by { seed, rounds, round_size }.
-- Challenges use ONE fixed format — not tied to a belt or a difficulty.
-- Questions are drawn from the WHOLE bank, all levels, round-robin by
-- category, so every duel is homogeneous and the ladder is comparable.
-- Both players replay the SAME seed and so get identical
...
-- ---- Realtime ----
-- Let clients subscribe to challenge changes (Supabase Realtime). The app
